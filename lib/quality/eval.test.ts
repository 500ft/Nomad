...
function topTitles(map: ReturnType<typeof buildV2ResearchMap>): string[] {
...
describe("v2 regression suite — 5 audit topics", () => {
  ...
  maybeIt("ai-mech-design: rejects hotel/hospitality/marketing drift from foundational picks", () => {
    const fx = loadFixture("ai-mech-design");
    if (!fx) return;
    const map = buildV2ResearchMap(fx.request, fx.works);
  ...
  maybeIt("battery-thermal: splits the mega-cluster into 2+ subclusters", () => {
  ...
  maybeIt("soft-robotics: keeps Mariana Trench / Magnetic Soft / Hydrogel papers in foundational", () => {
  ...
  maybeIt("am-defect: foundational picks stay in AM defect detection territory", () => {
  ...
  maybeIt("6g-channel: foundational picks include channel/RIS/terahertz papers", () => {
  ...
  maybeIt("relevance pipeline rejects ≥1 paper across all topics (sanity)", () => {
